...
import com.application.course_management.persistence.reports.AllEnrolledStudentsAndGradesReport;
...
public class AllEnrolledStudentsAndGradesReportDAO extends ReportDAO {
	
	private static final String COLLECTION_NAME = "all_students_and_grades_reports";
	public void storeReport(Teacher teacher, String reportName) throws UnknownHostException {
		...
		for(Course c : courses) {
			List<Enrollment> enrollments = c.getEnrollments().stream().filter(e -> {
				return e.getStatus().equals(EnrollmentService.STATUS_ACCEPTED);
