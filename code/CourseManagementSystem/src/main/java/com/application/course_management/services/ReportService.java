...
public class ReportService {
	...
	public void generateReport(String reportType, String reportName, Teacher teacher) {
		...
		switch (reportType) {
		case ALL_STUDENTS_AND_GRADES_REPORT:
			allStudentsAndGradesReportService.generateReport(teacher, reportName);
			break;
		case ALL_STUDENTS_BY_GROUPS_REPORT:
			allStudentsByGroupsReportService.generateReport(teacher, groups, reportName);
			break;
		case FAILING_STUDENTS_REPORT:
			failingStudentsReportService.generateReport(teacher, reportName);
			break;
		case PASSING_STUDENTS_REPORT:
			passingStudentsReportService.generateReport(teacher, reportName);
			break;
		}
	}
	...
	public List<Report> getReports(Teacher teacher, String reportType) {
		switch (reportType) {
		case ALL_STUDENTS_AND_GRADES_REPORT:
			return allStudentsAndGradesReportService.getReports(teacher);
		case ALL_STUDENTS_BY_GROUPS_REPORT:
			return allStudentsByGroupsReportService.getReports(teacher);
		case FAILING_STUDENTS_REPORT:
			return failingStudentsReportService.getReports(teacher);
		case PASSING_STUDENTS_REPORT:
			return passingStudentsReportService.getReports(teacher);
		default:
			return null;
		}
	}
